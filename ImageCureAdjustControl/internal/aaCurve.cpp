...
namespace aaAaa{
    ...
    std::string aaSpline::spline_name[SplineType::SPLINE_COUNT] = {"Cubic Spline",
																 "Quadratic Spline",
																 "Linear Spline"};
...
*/
	...
	aaCurve::aaCurve(const std::vector<std::pair<unsigned char, unsigned char> >& spline_data):
        m_knum(spline_data.size()), m_t(0), m_knot(0)
	{
		if (m_knum <= 0)
			return;
		...
		for (int i = 0; i < m_knum; ++i) {
			m_t[i] = spline_data[i].first;
			m_knot[i] = spline_data[i].second;
		}
	}
	...
	aaCubicSpline::aaCubicSpline(const std::vector<std::pair<unsigned char, unsigned char> >& spline_data):
        aaCurve(spline_data), m_ddp(0)
	{
		if (m_knum > 3)
			m_ddp = spline_cubic_set(m_knum, m_t, m_knot, 2, 0, 2, 0);
    ...
    bool aaCubicSpline::getValue(double t, double &value){
        ...
        if(m_knum > 3){
            ...
            double dp, ddp;
            value = spline_cubic_val(m_knum, m_t,m_knot,m_ddp,t,&dp,&ddp);
	...
	aaQuadraticSpline::aaQuadraticSpline(const std::vector<std::pair<unsigned char, unsigned char> >& spline_data)
        : aaCurve(spline_data)
    {}
	...
	aaLinearSpline::aaLinearSpline(const std::vector<std::pair<unsigned char, unsigned char> >& spline_data)
        : aaCurve(spline_data)
    {}
